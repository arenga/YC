---
layout: essay
title: "Write Like You Talk"
category: Mindset
difficulty: 중급
year: 
url_original: "https://paulgraham.com/talk.html"
---

## 요약 (Summary)

🎯 좋은 글쓰기의 비결은 무엇일까요?

✨ 핵심 내용 요약

Paul Graham의 간단한 조언: **말하듯이 쓰세요.**

우리는 학교에서 "격식 있는" 글쓰기를 배웠습니다. 수동태, 어려운 단어, 복잡한 문장. 하지만 이건 나쁜 글쓰기입니다.

좋은 글은 대화처럼 읽힙니다. 자연스럽고, 명확하고, 직설적입니다.

실천 방법:
1. 초고를 쓸 때, 친구에게 설명한다고 생각하세요
2. 수동태를 능동태로 바꾸세요
3. 어려운 단어를 쉬운 단어로 바꾸세요
4. 소리 내어 읽어보세요 - 어색하면 고치세요

왜 이게 어려울까요? 학교에서 반대로 배웠기 때문입니다. 하지만 이 습관을 버리면 훨씬 좋은 글을 쓸 수 있습니다.

**핵심 포인트**
• 좋은 글은 대화처럼 자연스럽습니다
• 격식, 수동태, 어려운 단어는 글을 나쁘게 만듭니다
• 소리 내어 읽어보면 나쁜 글쓰기가 드러납니다

🚀 오늘 바로 실천해볼 한 가지
최근 쓴 이메일이나 문서를 꺼내서 소리 내어 읽어보세요. 어색한 부분을 말하듯이 고쳐보세요.

---

## 한국어 번역 (Korean Translation)

2015년 10월당신이 쓴 글을 더 많은 사람들이 읽을 수 있도록 하는 간단한 방법은 다음과 같습니다. 말로 글을 쓰세요. 대부분의 사람들이 글을 쓰기 시작하면 뭔가 떠오르게 됩니다.그들은 친구와 대화할 때 사용하는 언어와는 다른 언어로 글을 씁니다.문장구조도 다르고 심지어 단어도 다릅니다.누구도 영어 구어체에서 "pen"을 동사로 사용하지 않습니다.친구와의 대화에서 "쓰기" 대신 "펜"을 사용하는 것이 바보처럼 느껴질 것입니다. 나에게 마지막 지푸라기는 며칠 전에 읽은 문장이었습니다. 변덕스러운 스페인 사람 자신이 다음과 같이 선언했습니다. "알타미라 이후에는 모든 것이 타락입니다."닐 올리버(Neil Oliver)의 고대 영국의 역사(A History of Ancient Britain)에 나오는 내용입니다.나는 이 책의 예를 만드는 것이 유감스럽다. 왜냐하면 이 책은 다른 책들보다 나쁘지 않기 때문이다.하지만 친구와 이야기할 때 피카소를 "변덕스러운 스페인 사람"이라고 부른다고 상상해 보세요.이 중 한 문장이라도 대화 중에 눈썹을 치켜 올릴 것입니다.그럼에도 불구하고 사람들은 그것에 관한 책 전체를 씁니다. 그렇군요, 글로 쓰는 언어와 말하는 언어는 다릅니다.그것이 서면 언어를 더 악화시키나요? 사람들이 당신이 쓴 것을 읽고 이해하기를 원한다면 그렇습니다.서면 언어는 더 복잡하기 때문에 읽기가 더 어렵습니다.또한 더 형식적이고 거리가 멀어 독자의 주의가 표류하도록 허용합니다.그러나 아마도 가장 나쁜 것은 복잡한 문장과 화려한 단어가 작가에게 실제보다 더 많은 것을 말하고 있다는 잘못된 인상을 줄 수 있다는 것입니다. 복잡한 생각을 표현하기 위해 복잡한 문장이 필요한 것은 아닙니다.어떤 난해한 주제의 전문가들이 자신의 분야에 대한 아이디어에 관해 서로 이야기할 때, 그들은 점심으로 무엇을 먹을지에 관해 이야기할 때보다 더 복잡한 문장을 사용하지 않습니다.그들은 확실히 다른 단어를 사용합니다.그러나 심지어 필요한 것 이상을 사용하지도 않습니다.그리고 내 경험상, 주제가 어려울수록 전문가들은 더 비공식적으로 이야기합니다.부분적으로는 증명할 것이 적고 부분적으로는 이야기하는 아이디어가 어려울수록 언어를 방해할 여유가 없기 때문이라고 생각합니다. 비공식 언어는 아이디어의 운동복입니다. 말로 하는 언어가 항상 가장 효과적이라고 말하는 것은 아닙니다.시는 텍스트만큼이나 음악이므로 대화에서는 말하지 않을 것들을 말할 수 있습니다.그리고 산문에서 화려한 언어를 사용해도 무사히 빠져나갈 수 있는 소수의 작가들이 있습니다.그리고 물론 작가들이 자신이 말하는 내용을 쉽게 이해하기를 원하지 않는 경우도 있습니다. 예를 들어 회사에서 나쁜 소식을 발표할 때나 인문학의 가장 가짜 부분에서 말입니다.그러나 거의 모든 사람들에게는 말로 하는 것이 더 좋습니다. 대부분의 사람들이 말로 글을 쓰는 것은 어려운 것 같습니다.따라서 아마도 가장 좋은 해결책은 평소와 같이 첫 번째 초안을 작성한 다음 각 문장을 살펴보고 "내가 친구와 이야기할 때 이렇게 말할 것인가?"라고 질문하는 것입니다.그렇지 않다면 무엇을 말할지 상상하고 대신 사용하십시오.잠시 후 이 필터는 글을 쓰는 동안 작동하기 시작합니다.말하지 않을 글을 쓰면 페이지에 닿을 때 딸깍거리는 소리가 들립니다. 저는 새 에세이를 출판하기 전에 큰 소리로 읽고 대화처럼 들리지 않는 모든 것을 고쳐줍니다.심지어 발음상 어색한 부분도 수정합니다.꼭 필요한지는 모르겠지만 비용이 많이 들지는 않습니다. 이 방법만으로는 충분하지 않을 수도 있습니다.나는 글쓰기가 구어와 너무 동떨어져서 문장 하나하나를 고칠 수 없는 것을 보았습니다.그러한 경우에는 더 과감한 해결책이 있습니다.첫 번째 초안을 작성한 후 방금 쓴 내용을 친구에게 설명해보세요.그런 다음 친구에게 말한 내용으로 초안을 교체하세요. 사람들은 내 에세이가 얼마나 내가 말하는 것처럼 들린다고 자주 말합니다.이것이 논평할만한 가치가 있는 것처럼 보인다는 사실은 사람들이 말로 글을 쓰는 일이 얼마나 드물다는 것을 보여줍니다.그렇지 않으면 모든 사람의 글이 그들이 말하는 것처럼 들릴 것입니다. 단순히 말로 글을 쓴다면 당신은 95%의 작가보다 앞서게 될 것입니다.그리고 하기가 너무 쉽습니다. 친구에게 말하는 방식이 아닌 이상 문장을 끝까지 읽지 마세요. 초안을 읽어주신 Patrick Collison과 Jessica Livingston에게 감사드립니다.일본어 Tr

번역아랍어 번역

---

## 원문 (Original Essay)

October 2015Here's a simple trick for getting more people to read what you write: write in spoken language.Something comes over most people when they start writing. They write in a different language than they'd use if they were talking to a friend. The sentence structure and even the words are different. No one uses "pen" as a verb in spoken English. You'd feel like an idiot using "pen" instead of "write" in a conversation with a friend.The last straw for me was a sentence I read a couple days ago: The mercurial Spaniard himself declared: "After Altamira, all is decadence." It's from Neil Oliver's A History of Ancient Britain. I feel bad making an example of this book, because it's no worse than lots of others. But just imagine calling Picasso "the mercurial Spaniard" when talking to a friend. Even one sentence of this would raise eyebrows in conversation. And yet people write whole books of it.Ok, so written and spoken language are different. Does that make written language worse?If you want people to read and understand what you write, yes. Written language is more complex, which makes it more work to read. It's also more formal and distant, which gives the reader's attention permission to drift. But perhaps worst of all, the complex sentences and fancy words give you, the writer, the false impression that you're saying more than you actually are.You don't need complex sentences to express complex ideas. When specialists in some abstruse topic talk to one another about ideas in their field, they don't use sentences any more complex than they do when talking about what to have for lunch. They use different words, certainly. But even those they use no more than necessary. And in my experience, the harder the subject, the more informally experts speak. Partly, I think, because they have less to prove, and partly because the harder the ideas you're talking about, the less you can afford to let language get in the way.Informal language is the athletic clothing of ideas.I'm not saying spoken language always works best. Poetry is as much music as text, so you can say things you wouldn't say in conversation. And there are a handful of writers who can get away with using fancy language in prose. And then of course there are cases where writers don't want to make it easy to understand what they're saying—in corporate announcements of bad news, for example, or at the more bogus end of the humanities. But for nearly everyone else, spoken language is better.It seems to be hard for most people to write in spoken language. So perhaps the best solution is to write your first draft the way you usually would, then afterward look at each sentence and ask "Is this the way I'd say this if I were talking to a friend?" If it isn't, imagine what you would say, and use that instead. After a while this filter will start to operate as you write. When you write something you wouldn't say, you'll hear the clank as it hits the page.Before I publish a new essay, I read it out loud and fix everything that doesn't sound like conversation. I even fix bits that are phonetically awkward; I don't know if that's necessary, but it doesn't cost much.This trick may not always be enough. I've seen writing so far removed from spoken language that it couldn't be fixed sentence by sentence. For cases like that there's a more drastic solution. After writing the first draft, try explaining to a friend what you just wrote. Then replace the draft with what you said to your friend.People often tell me how much my essays sound like me talking. The fact that this seems worthy of comment shows how rarely people manage to write in spoken language. Otherwise everyone's writing would sound like them talking.If you simply manage to write in spoken language, you'll be ahead of 95% of writers. And it's so easy to do: just don't let a sentence through unless it's the way you'd say it to a friend.Thanks to Patrick Collison and Jessica Livingston for reading drafts of this.Japanese TranslationArabic Translation

---

_분석일: 2025. 11. 29._
_수집일: 2025. 11. 28._