...
최근 강연에서 저는 많은 사람들을 화나게 하는 말을 했습니다. 즉, Java 프로젝트에서 작업하는 것보다 Python 프로젝트에서 작업하는 더 똑똑한 프로그래머를 얻을 수 있다는 것입니다.

Java 프로그래머가 멍청하다는 뜻은 아닙니다. 제 말은 Python 프로그래머가 똑똑하다는 뜻이었습니다. 새로운 프로그래밍 언어를 배우는 것은 많은 노력이 필요합니다. 그리고 사람들은 직업을 얻을 수 있기 때문에 Python을 배우지 않습니다. 그들은 진정으로 프로그래밍을 좋아하고 자신이 이미 알고 있는 언어에 만족하지 않기 때문에 그것을 배웁니다.

이것이 바로 그들이 회사에서 채용하고 싶어하는 유형의 프로그래머가 되는 이유입니다. 따라서 더 나은 이름이 없기 때문에 저는 Python 역설이라고 부르겠습니다: 회사가 소프트웨어를 비교적 난해한 언어로 작성하기로 선택하면 더 나은 프로그래머를 고용할 수 있습니다. 왜냐하면 Python을 배우려는 관심을 가진 사람들만 끌어들일 것이기 때문입니다.

그리고 프로그래머들에게 있어 역설은 훨씬 더 뚜렷합니다. 좋은 직업을 얻기 위해 배워야 할 언어는 사람들이 단순히 직업을 얻기 위해 배우는 것이 아닌 언어입니다.

지금까지 이것을 깨달을 만큼 똑똑한 회사는 소수에 불과합니다. 그러나 여기에도 일종의 선택이 진행되고 있습니다. 바로 프로그래머가 가장 일하고 싶어하는 회사입니다. 예를 들어 구글입니다. Java 프로그래밍 작업을 광고할 때 Python 경험도 원합니다.

널리 사용되는 거의 모든 언어를 아는 제 친구는 대부분의 프로젝트에서 Python을 사용합니다. 그는 주된 이유가 소스 코드의 모양이 마음에 들기 때문이라고 말했습니다. 그것은 다른 언어보다 한 언어를 선택하는 경솔한 이유처럼 보일 수 있습니다. 그러나 그것은 들리는 것처럼 그렇게 경솔하지 않습니다.

프로그래밍할 때 코드를 작성하는 것보다 읽는 데 더 많은 시간을 소비합니다. 조각가가 점토 덩어리를 만지는 것처럼 소스 코드 덩어리를 밀어넣습니다. 따라서 소스 코드를 추악하게 만드는 언어는 엄격한 프로그래머를 미치게 만듭니다. 마치 덩어리진 점토가 조각가를 미치게 만드는 것과 같습니다.

추악한 소스 코드를 언급하면 사람들은 당연히 Perl을 떠올릴 것입니다. 그러나 Perl의 표면적인 추악함은 제가 말하는 그런 종류의 것이 아닙니다. 진짜 추악함은 거칠어 보이는 구문이 아니라 잘못된 개념으로 프로그램을 구축해야 하는 것입니다. Perl은 만화 캐릭터가 욕하는 것처럼 보일 수도 있지만 개념적으로는 Python을 능가하는 경우도 있습니다.

어쨌든 여기까지는 그렇습니다. 물론 두 언어 모두 움직이는 목표물입니다. 그러나 그들은 Ruby (및 Icon, Joy, J, Lisp, Smalltalk)와 함께 프로그래밍에 진정으로 관심이 있는 사람들이 만들고 사용한다는 사실을 공유합니다. 그리고 그런 사람들은 그 일을 잘하는 경향이 있습니다.
