...
**난이도**: 중급
**출판일**: N/A
**원문**: https://paulgraham.com/want.html
---
## 요약 (Summary)
...
2022년 11월 저는 9살 때부터 예측 가능한 방식으로 움직이는 물질로 만들어졌다는 것과 제가 원하는 것은 무엇이든 선택할 수 있다는 느낌 사이의 명백한 모순에 대해 의아해했습니다.그 당시 나는 그 질문을 탐구하려는 이기적인 동기를 가지고 있었습니다.그 나이에(대부분의 후세와 마찬가지로) 나는 항상 당국과 문제를 겪었고, 내 행동에 책임이 없다고 주장함으로써 문제에서 벗어날 수 있는 방법이 있을 것 같았습니다.나는 점차 희망을 잃었지만 문제는 남아 있었습니다. 물질로 이루어진 기계라는 존재와 자신이 하는 일을 자유롭게 선택할 수 있다는 느낌을 어떻게 조화시킬 수 있을까요?[1]답변을 설명하는 가장 좋은 방법은 약간 잘못된 버전으로 시작한 다음 수정하는 것입니다.잘못된 버전은 다음과 같습니다. 원하는 것을 할 수 있지만 원하는 것을 원할 수는 없습니다.예, 당신은 당신이 하는 일을 통제할 수 있지만 당신이 원하는 것을 할 것이고 그것을 통제할 수는 없습니다. 이것이 잘못된 이유는 사람들이 때때로 자신이 원하는 것을 바꾸기 때문입니다.무언가를 원하지 않는 사람들(예를 들어 마약 중독자)은 때때로 스스로 그것을 원하지 않게 만들 수 있습니다.그리고 무언가를 원하는 사람들, 즉 클래식 음악이나 브로콜리를 좋아하는 사람들은 때때로 성공합니다. 그래서 우리는 초기 진술을 수정합니다. 원하는 것을 할 수 있지만 원하는 것을 원할 수는 없습니다. 그것은 여전히 ​​사실이 아닙니다.원하시는 내용으로 변경 가능합니다.누군가가 "클래식 음악을 더 이상 좋아하지 않기로 결정했습니다"라고 말하는 모습을 상상할 수 있습니다.하지만 우리는 진실에 점점 가까워지고 있습니다.사람들이 자신이 원하는 것을 바꾸는 일은 거의 없으며, "원하는 것"을 더 많이 추가할수록 더 드물어집니다. 소수점 이하 9의 문자열에 9를 더 추가하여 임의로 1에 가까워질 수 있는 것과 거의 같은 방식으로 "원하는 것"을 더 많이 추가하면 임의로 실제 진술에 가까워질 수 있습니다.실제로는 3~4개의 "원하는 것"이면 충분합니다.실제로 하고 싶은 것을 바꾸는 것은 커녕, 원하는 것을 바꾼다는 것이 무엇을 의미할지 상상조차 하기 어렵습니다. 따라서 정답을 표현하는 한 가지 방법은 정규식을 사용하는 것입니다.당신은 당신이 원하는 것을 할 수 있지만, "당신은 당신이 원하는 것을 할 수 없다(원한다)* 원한다"라는 표현이 사실입니다.결국 당신은 통제할 수 없는 욕구로 돌아가게 됩니다.[2] 참고[1] 나는 9살 때 물질이 무작위로 행동할 수 있다는 것을 몰랐지만 그것이 문제에 큰 영향을 미치지는 않는다고 생각합니다.무작위성은 결정론만큼 효과적으로 기계 속의 유령을 파괴합니다.[2]표현 사용이 마음에 들지 않으면 고차 욕구를 사용하여 동일한 요점을 만들 수 있습니다. n차 욕구를 제어할 수 없는 n이 있습니다.초안을 읽어주신 Trevor Blackwell, Jessica Livingston, Robert Morris, Michael Nielsen에게 감사드립니다.
